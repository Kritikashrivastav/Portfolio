...
const LandingPage = () => {
  ...
  return (
    <div
      ...
      data-scroll-section
      data-scroll-speed="-.3"
      className="bg-zinc-900 w-full min-h-screen pt-1 relative overflow-hidden flex flex-col justify-between"
    >
      <div
      ...
      <div className="textstructure mt-40 md:mt-64 flex items-center md:items-start flex-col md:flex-col gap-2 md:gap-0 md:px-20 pb-5 sm:mb-10 md:mb-32">
        {headerItems.map((item, index) => (
          <div key={index} className="masker">
            <h1
              className={`uppercase tracking-tighter font-monoton md:font-audiowide ${
                index === 2
      ...
      {/* Footer items */}
      <div className="hidden md:flex border-t-[1px] font-poppins border-zinc-800 flex-row justify-between items-center pt-7 px-10 md:px-20 mt-auto">
        {footerItems.map((item, index) => (
          <p key={index} className="text-base font-light sm:mx-2 flex-1 text-center">
            {item}
          </p>
        ))}
      </div>
